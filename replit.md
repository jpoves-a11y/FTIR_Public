...
## Trial Usage Limit System
The application includes a trial limit system that restricts usage for new users. The system uses server-side validation with Cloudflare Pages Functions and KV storage to prevent easy bypassing.

### Movement Costs
- **File Loading**: 1 move
- **Individual Analyses** (Oxidation, Crystallinity, TVI, VitaminE): 1 move each
- **Full Auto Analysis**: 1 move (includes all sub-analyses)
- **Individual Deconvolution**: 1 move
- **Multiple Deconvolution**: 2 moves
- **Exports** (CSV, Excel, JSON, PDF): 2 moves each

### Server-Side Validation (Cloudflare Pages Functions)
The trial system now uses a backend API deployed as Cloudflare Pages Functions:
- **Endpoint**: `/api/trial` (handled by `functions/api/trial.js`)
- **Storage**: Cloudflare KV namespace `TRIAL_KV`
- **Key Generation**: SHA-256 hash of (fingerprint + client IP + salt)

This prevents bypassing via:
- Private/Incognito browsing (same IP is tracked server-side)
- Different browser (same IP is tracked)
- Clearing browser data (data is stored server-side)
- Different email account (IP + fingerprint remains similar)

### Device Fingerprinting
Uses multiple signals for user identification:
- Canvas fingerprint
- WebGL fingerprint
- Audio context fingerprint
- Screen/timezone/navigator data

### Redundant Storage (Fallback)
If the server is unavailable, trial data falls back to local storage layers:
- localStorage
- IndexedDB
- Cache API
- sessionStorage

### Hidden Admin Reset (Local Only)
Administrators can reset the local trial limit using:
- **Trigger 1**: Ctrl+Shift + 5 clicks on GBM logo
- **Trigger 2**: Ctrl+Shift+Alt+R keyboard shortcut
- **Password**: pjce2025
- **Cooldown**: 5 minutes between resets

Note: Admin reset only affects local storage, not server-side data.

### Cloudflare Setup
See `CLOUDFLARE_SETUP.md` for detailed instructions on configuring Cloudflare KV.
